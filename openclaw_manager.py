...
class OpenClawManager:
    ...
    def list_instances(self, verbose: bool = False):
        ...
        print(
            f"\n{'Instance':<15} {'Port':<8} {'Status':<10} {'Autostart':<10} {'Model'}"
        )
        print("-" * 70)
    ...
    def install_aliases(self) -> bool:
        ...
        if not shell_rc.exists():
            print("Error: Could not find .bashrc or .zshrc")
            return False

        alias_block = """
# OpenClaw Multi-Instance Manager Aliases
alias ocm='python3 ~/openclaw_manager.py'
alias ocml='ocm list'
alias ocms='ocm start'
alias ocmsp='ocm stop'
alias ocmr='ocm restart'
alias ocmc='ocm create'
alias ocmd='ocm delete --force'
alias ocmst='ocm status'
alias ocmh='ocm health'
alias ocmlg='ocm logs'
alias ocmb='ocm backup'
alias ocmrs='ocm restore'
alias ocmu='ocm use'           # Run openclaw command in instance context
alias ocment='ocm enter'       # Interactive shell for instance
"""

        # Check if already installed
        with open(shell_rc, "r") as f:
            content = f.read()
            if "OpenClaw Multi-Instance Manager Aliases" in content:
                print("Aliases already installed in " + str(shell_rc))
                return True
        ...
        print(f"✓ Aliases installed in {shell_rc}")
        print("\nAvailable aliases:")
        print("  ocm     - OpenClaw Manager")
        print("  ocml    - List instances")
        print("  ocms    - Start instance")
        print("  ocmsp   - Stop instance")
        print("  ocmr    - Restart instance")
        print("  ocmc    - Create instance")
        print("  ocmd    - Delete instance")
        print("  ocmst   - Show status")
        print("  ocmh    - Health check")
        print("  ocmlg   - View logs")
        print("  ocmb    - Backup instance")
        print("  ocmrs   - Restore instance")
        print("\nRun 'source " + str(shell_rc) + "' to use aliases now")
        return True
...
Examples:
  ...
  %(prog)s delete worker1 --force
    ...
    subparsers.add_parser(
        "install-aliases", help="Install shell aliases for quick access"
    )
    ...
    if args.command == "use":
        ...
        commands = {
            ...
            "install-aliases": lambda: manager.install_aliases(),
        }
